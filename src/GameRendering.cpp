...
{
    createTiles(game, m_tiles, m_walls, m_arch); // dans surcouche
...
void GameRendering::drawArch()
{
    for (size_t i = 0; i < m_arch.size(); i++) {
        m_arch[i]->draw(m_globalProjMatrix, m_globalMvMatrix);
    }
}
